...
<div class="max-w-5xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="font-serif text-3xl font-semibold text-slate-900 tracking-tight">Mes emprunts en cours</h1>
            <p class="text-slate-500 mt-1">Gérez vos lectures actuelles et prolongez vos prêts si besoin.</p>
        </div>
        <a href="{{ route('adherent.prets.history') }}" class="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-editorial bg-white border border-slate-200 px-4 py-2 rounded-full transition-colors shadow-sm">
    ...
    @if($prets->isEmpty())
        <div class="bg-white rounded-[2rem] p-12 text-center shadow-sm border border-slate-200/60 border-dashed">
            ...
            <h2 class="font-serif text-2xl font-semibold text-slate-900 mb-2">Vous n'avez aucun prêt en cours</h2>
            ...
            <a href="{{ route('adherent.catalogue.index') }}" class="inline-flex items-center gap-2 px-8 py-3.5 bg-editorial hover:bg-editorial-light text-white font-medium rounded-xl transition-colors shadow-sm shadow-editorial/20">
                    ...
                    <div class="flex-1 flex flex-col justify-between">
                        <div>
                            <div class="flex items-start justify-between gap-2 mb-1">
                                <h3 class="font-serif font-bold text-xl text-slate-900 leading-tight">
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                        <div class="h-full rounded-full {{ $enRetard ? 'bg-red-500 w-full' : ($pourcentage > 80 ? 'bg-amber-500' : 'bg-editorial') }}" style="width: {{ $enRetard ? '100%' : $pourcentage . '%' }}"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 flex items-center gap-3">
                            @if(!$enRetard && !$pret->date_retour_reelle)
                                <form action="{{ route('adherent.prets.prolonger', $pret->id) }}" method="POST" class="flex-1">
                                    @csrf
                                    <button type="submit" class="w-full px-4 py-2 bg-white border border-slate-200 hover:border-editorial text-slate-700 hover:text-editorial text-sm font-medium rounded-xl shadow-sm transition-all text-center flex items-center justify-center gap-2" onclick="return confirm('Voulez-vous vraiment prolonger ce prêt ?')">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                        Prolonger
                                    </button>
                                </form>
                            @endif
                            <a href="{{ route('adherent.catalogue.show', $pret->exemplaire->ouvrage->id) }}" class="{{ $enRetard ? 'w-full flex-1' : '' }} px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 text-sm font-medium rounded-xl transition-colors text-center">
                                Détails
                            </a>
                        </div>
                    </div>
