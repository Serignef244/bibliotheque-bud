...
<div class="max-w-5xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
            <h1 class="font-serif text-3xl font-semibold text-slate-900 tracking-tight">Mon Profil</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            <!-- Digital Card -->
            <div class="bg-gradient-to-br from-editorial to-slate-900 rounded-[2rem] p-8 shadow-xl text-white relative overflow-hidden group">
                <!-- Abstract BG -->
                <div class="absolute -right-10 -top-10 w-48 h-48 bg-white/10 rounded-full blur-2xl"></div>
                <div class="absolute -left-10 -bottom-10 w-48 h-48 bg-editorial-light/20 rounded-full blur-2xl"></div>

                <div class="relative z-10">
                    <div class="flex justify-between items-start mb-12">
                        <div>
                            <span class="font-serif font-bold text-2xl tracking-tight">Biblio<span class="text-white/70">Num</span></span>
                    ...
                    <div class="space-y-1">
                        <p class="font-serif text-xl font-medium tracking-wide">{{ auth()->user()->name }}</p>
            ...
            <!-- Download Button -->
            <a href="{{ route('adherent.profil.carte') }}" class="w-full py-3.5 bg-white border border-slate-200 hover:border-editorial text-slate-700 hover:text-editorial text-sm font-medium rounded-2xl shadow-sm transition-all flex items-center justify-center gap-2">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200/60 overflow-hidden">
                <div class="p-8 border-b border-slate-100 flex items-center justify-between">
                    <h2 class="font-serif text-xl font-semibold text-slate-900">Informations personnelles</h2>
                    <button class="text-editorial hover:text-editorial-light text-sm font-medium" onclick="alert('La modification de profil sera bientôt disponible !')">Modifier</button>
                </div>
                ...
                <div class="bg-slate-50 p-8 border-t border-slate-100">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-white p-4 rounded-xl border border-slate-200/50">
                            <span class="block text-2xl font-serif font-bold text-slate-900 mb-1">{{ $adherent->prets()->count() }}</span>
                            <span class="text-xs font-medium text-slate-500 uppercase tracking-wider">Livres empruntés</span>
                        </div>
                        <div class="bg-white p-4 rounded-xl border border-slate-200/50">
                            <span class="block text-2xl font-serif font-bold text-slate-900 mb-1">{{ $adherent->penalites()->count() }}</span>
