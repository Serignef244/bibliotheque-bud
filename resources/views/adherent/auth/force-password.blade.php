...
<div class="max-w-2xl mx-auto py-12">
    <div class="bg-white rounded-[2rem] p-8 sm:p-12 shadow-sm border border-slate-200/60 text-center mb-8 relative overflow-hidden">
        <div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-editorial to-editorial-light"></div>
        ...
        <h1 class="font-serif text-3xl font-bold text-slate-900 mb-4">Sécurisez votre compte</h1>
        <p class="text-slate-500 leading-relaxed max-w-lg mx-auto">
            Bienvenue sur votre espace adhérent ! Pour des raisons de sécurité, vous devez personnaliser votre mot de passe généré automatiquement avant de pouvoir continuer.
        </p>
    </div>

    <form action="{{ route('adherent.password.force-change.store') }}" method="POST" class="bg-white rounded-[2rem] p-8 sm:p-10 shadow-sm border border-slate-200/60">
        @csrf
        ...
        <div class="mt-10 pt-6 border-t border-slate-100 flex items-center justify-between">
            ...
            <button type="submit" class="px-8 py-3 bg-editorial hover:bg-editorial-light text-white font-medium rounded-xl transition-colors shadow-sm shadow-editorial/20 flex items-center gap-2">
