...
<div class="max-w-2xl mx-auto">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500 mb-6">
        ...
        <span class="text-slate-800 font-medium">{{ $categorie->nom }}</span>
    </nav>

    <form method="POST" action="{{ route('admin.categories.update', $categorie) }}" class="space-y-5">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-5">
            ...
            <div>
                <label for="nom" class="block text-sm font-medium text-slate-700 mb-1.5">Nom <span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <select id="parent_id" name="parent_id"
                    ...
                    @foreach($parentes as $p)
                        @if($p['id'] !== $categorie->id) {{-- Empêcher de se sélectionner soi-même --}}
                        <option value="{{ $p['id'] }}" {{ old('parent_id', $categorie->parent_id) == $p['id'] ? 'selected' : '' }}>
                            {{ $p['label'] }}
                        </option>
                        @endif
                    @endforeach
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition resize-none">{{ old('description', $categorie->description) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="ordre" class="block text-sm font-medium text-slate-700 mb-1.5">Ordre d'affichage</label>
                    <input type="number" id="ordre" name="ordre" value="{{ old('ordre', $categorie->ordre) }}" min="0"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition">
                </div>
