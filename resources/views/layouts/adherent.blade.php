...
<body class="bg-editorial-50 text-slate-900 font-sans selection:bg-editorial-light selection:text-white">
    ...
    <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
        ...
        <!-- Sidebar -->
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-200 transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:flex-shrink-0 flex flex-col">
            
            <!-- Logo -->
            <div class="h-20 flex items-center px-8 border-b border-slate-100">
                <a href="{{ route('adherent.dashboard') }}" class="flex items-center gap-3 group">
                    <div class="w-10 h-10 bg-editorial rounded-xl flex items-center justify-center text-white font-serif font-bold text-xl shadow-sm group-hover:scale-105 transition-transform">
                        B
                    </div>
                    <span class="font-serif font-semibold text-xl tracking-tight text-slate-900">Biblio<span class="text-editorial-light">Num</span></span>
                </a>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto px-4 py-6 space-y-1">
                ...
                <p class="px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 mt-4">Ma bibliothèque</p>
                
                @foreach ($navItems as $item)
                    <a href="{{ Route::has($item['route']) ? route($item['route']) : '#' }}" 
                       class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 group {{ request()->routeIs($item['route'].'*') ? 'bg-editorial text-white shadow-md shadow-editorial/20' : 'text-slate-600 hover:bg-slate-50 hover:text-editorial' }}">
                        <svg class="w-5 h-5 {{ request()->routeIs($item['route'].'*') ? 'text-white' : 'text-slate-400 group-hover:text-editorial' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            {!! $item['icon'] !!}
                        </svg>
                ...
                <p class="px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 mt-8">Mon compte</p>
                
                <a href="{{ Route::has('adherent.profil.index') ? route('adherent.profil.index') : '#' }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 group {{ request()->routeIs('adherent.profil.*') ? 'bg-editorial text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-editorial' }}">
                    <svg class="w-5 h-5 text-slate-400 group-hover:text-editorial" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    <span class="font-medium text-sm">Mon profil</span>
                </a>
                
                <a href="{{ Route::has('adherent.parametres.index') ? route('adherent.parametres.index') : '#' }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 group text-slate-600 hover:bg-slate-50 hover:text-editorial">
                    <svg class="w-5 h-5 text-slate-400 group-hover:text-editorial" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            ...
            <div class="p-4 border-t border-slate-100">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors group">
                        <svg class="w-5 h-5 text-slate-400 group-hover:text-red-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
        ...
        <!-- Main Wrapper -->
        <div class="flex-1 flex flex-col h-screen overflow-hidden bg-editorial-50">
            ...
            <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-30 px-4 sm:px-8 flex items-center justify-between">
                ...
                <div class="flex items-center gap-4 sm:gap-6">
                    <!-- Notifications -->
                    <a href="{{ Route::has('adherent.notifications.index') ? route('adherent.notifications.index') : '#' }}" class="relative p-2 text-slate-400 hover:text-editorial transition-colors rounded-full hover:bg-slate-50">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <img class="h-10 w-10 rounded-full object-cover border-2 border-white shadow-sm" src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=1E3A8A&color=fff" alt="">
                    </div>
                </div>
            </header>
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-8">
                ...
                <footer class="mt-12 py-6 border-t border-slate-200/60 text-center">
                    <p class="text-sm text-slate-500">© {{ date('Y') }} Bibliothèque Numérique BUD. Tous droits réservés.</p>
                </footer>
            </main>
